...
class LinearLoss(object):
    ...
    def loss(self, predictions, policy, cfv):
        return (-tf.reduce_mean(cfv * predictions) + tf.reduce_mean(
...
class InstRegretStratMatching(object):
    ...
    def loss(self, predictions, policy, cfv):
        r = tf.stop_gradient(
            rm_policy(
                cfv - tf.reduce_sum(cfv * policy, axis=1, keepdims=True)))
        log_policy = tf.log(tf.clip_by_value(policy, 1e-15, 1 - 1e-15))
...
class SplitRrm(MetaRmp):
    ...
    def loss(self, predictions, policy, cfv):
        ...
        ev = tf.stop_gradient(
            tf.reduce_sum(cfv * pi_rm, axis=1, keepdims=True))
